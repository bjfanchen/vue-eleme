    ...
    created () {
      axios.get('static/data.json').then((res) => {
          ...
          this.$nextTick(() => {
            ...
            this._calculateHeight() // 初始化列表高度列表，实现左右联动
          })
        }
      )
    },
    ...
    computed: {
      menuCurrentIndex() {
        for (let i = 0, l = this.listHeight.length; i < l; i++) {
          ...
          let bottomHeight = this.listHeight[i + 1] // 最后一个返回undefined
    ...
    methods: {
      ...
      _calculateHeight () {
        ...
        for (let i = 0, l = foodList.length; i < l; i++) {
          let item = foodList[i]
          height += item.clientHeight // 通过clientHeight获取元素高度
          this.listHeight.push(height)
        }
      },
